...
<title>字典表_编辑页面_Jangle生成工具v1.1</title>
...
<body>
	<div id="bodyTopDiv" class="jangle-editjsp-bodytopdiv" >
		<span class="label label-info">字典表-内容编辑</span>
	...
	<div class="jangle-editjsp-formdiv" >
		<form id="jangleEditForm" name="jangleEditForm" class="formJ" >
			...
			<div class="form-group" >
				...
				<input type="text" class="form-control" id="dkey" name="dkey" placeholder="请输入dkey" />
			</div>
			<div class="form-group" >
				<label for="dvalue">dvalue：</label>
				<input type="text" class="form-control" id="dvalue" name="dvalue" placeholder="请输入dvalue" />
			</div>
			<div class="form-group">
	...
	<script type="text/javascript" src="js/bsDictEdit.js"></script>
	<script type="text/javascript">
			...
			$.ajax({
				...
				success : function(data) {
					...
					if (data.code == "10001") {
						...
						fileInputParam["attSourceType"] = "bs_dict"; //填业务表名称
		...
		function deleteForm() {
			if (confirm("确定删除此记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/bsDictCtrl/deleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"id" : $("#id").val()
		...
		$(function() {
			...
			$.ajax({
				url : "/bsDictCtrl/selectByPrimaryKey.ctrl",
				dataType : "json",
				cache : false,
				data : {
					"id" : ps["id"]
