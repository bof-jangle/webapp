...
<title>_编辑页面_Jangle生成工具v1.1</title>
...
<body>
	<div id="bodyTopDiv" class="jangle-editjsp-bodytopdiv" >
		<span class="label label-info">-内容编辑</span>
	...
	<div class="jangle-editjsp-formdiv" >
		<form id="jangleEditForm" name="jangleEditForm" class="formJ" >
			<div class="form-group" >
				...
				<input type="text" class="form-control" id="bsUserUuid" name="bsUserUuid" placeholder="请输入用户UUID" />
			</div>
			<div class="form-group" >
				<label for="bsRoleId">角色ID：</label>
				<input type="text" class="form-control" id="bsRoleId" name="bsRoleId" placeholder="请输入角色ID" />
			</div>
			<div class="form-group" >
				<label for="dmDesc">dm_desc：</label>
				<input type="text" class="form-control" id="dmDesc" name="dmDesc" placeholder="请输入dm_desc" />
			</div>
			<div class="form-group" >
				<label for="dmDesc2">dm_desc2：</label>
				<input type="text" class="form-control" id="dmDesc2" name="dmDesc2" placeholder="请输入dm_desc2" />
			</div>
			<div class="form-group">
	...
	<script type="text/javascript" src="js/bsRoleUserREdit.js"></script>
	<script type="text/javascript">
		...
		function submitForm() {
			...
			$.ajax({
				type:"POST",
				url : "/bsRoleUserRCtrl/insert.ctrl",
				dataType : "json",
				cache:false,
				...
				success : function(data) {
					...
					if (data.code == "10001") {
						...
						fileInputParam["attSourceType"] = "bs_role_user_r"; //填业务表名称
		...
		function deleteForm() {
			if (confirm("确定删除此记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/bsRoleUserRCtrl/deleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"id" : $("#id").val()
