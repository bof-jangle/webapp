...
<title>访问记录表_编辑页面_Jangle生成工具v1.1</title>
...
<body>
	<div id="bodyTopDiv" class="jangle-editjsp-bodytopdiv" >
		<span class="label label-info">访问记录表-内容编辑</span>
	...
	<script type="text/javascript" src="js/bsVisitEdit.js"></script>
	<script type="text/javascript">
		...
		function deleteForm() {
			if (confirm("确定删除此记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/bsVisitCtrl/deleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"id" : $("#id").val()
		...
		$(function() {
			...
			$.ajax({
				url : "/bsVisitCtrl/selectByPrimaryKey.ctrl",
				dataType : "json",
				cache : false,
				data : {
					"id" : ps["id"]
