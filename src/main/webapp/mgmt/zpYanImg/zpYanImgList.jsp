...
<!-- 	    </div> -->
	    <div>
	        <form class="navbar-form" role="search" id="searchForm">
	            <div class="form-group">
	            	<span class="jangle-listjsp-remarks-span label label-info" >yan的作品 </span>
			...
			<table id="tablewrap" data-toggle="table" data-locale="zh-CN"
					...
					<thead style="text-align: center;">
						<tr>
							...
							<th data-field="dmDesc" >描述</th>
							<th data-field="createTime" data-formatter="datetimeFormatterJ" data-width="200" >创建时间</th>
							<th data-field="id" data-width="100" data-formatter="operateFormatOpen">操作</th>
						</tr>
					</thead>
				</table>
	</div>
	...
	<script type="text/javascript">
		...
		var url = "/zpYanImgCtrl/selectPage.ctrl";	//获取数据的url地址，需要实现分页功能。
		// 新增按钮 打开新增数据的页面
		function addFormInfo() {
			window.location.href = "zpYanImgEdit.jsp"
		}
		// 查看详情
		function openDetail(data) {
			window.location.href = "zpYanImgOpen.jsp?id=" + data.id + addressPostfix;
		}
		// 编辑详情
		function editDetail(data){
			window.location.href = "zpYanImgEdit.jsp?id=" + data.id + addressPostfix;
		}
		// 复选框的相关功能（当启用复选框时可用）
		function doCheckBoxes(){
			...
			if (confirm("确定删除勾选的"+rows.length+"条记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/zpYanImgCtrl/batchDeleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"ids" : ids
