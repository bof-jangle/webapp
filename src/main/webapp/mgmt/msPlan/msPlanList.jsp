	    ...
	    <div>
	        <form class="navbar-form" role="search" id="searchForm">
	            <div class="form-group">
	            	<span class="jangle-listjsp-remarks-span label label-info" >面试安排 </span>
			...
			<table id="tablewrap" data-toggle="table" data-locale="zh-CN"
					...
					<thead style="text-align: center;">
						<tr>
							...
							<th data-field="work" >职务</th>
							<th data-field="address" >地址</th>
							<th data-field="myPlan" >安排</th>
<!-- 							<th data-field="createTime" data-formatter="datetimeFormatterJ" data-width="150">创建时间</th> -->
	...
	<script type="text/javascript">
		...
		var url = "/msPlanCtrl/selectPage.ctrl";	//获取数据的url地址，需要实现分页功能。
		// 新增按钮 打开新增数据的页面
		function addFormInfo() {
			window.location.href = "msPlanEdit.jsp"
		}
		// 查看详情
		function openDetail(data) {
			window.location.href = "msPlanOpen.jsp?id=" + data.id + addressPostfix;
		}
		// 编辑详情
		function editDetail(data){
			window.location.href = "msPlanEdit.jsp?id=" + data.id + addressPostfix;
		}
		// 行状态色彩格式化
		...
		function doCheckBoxes(){
			...
			if (confirm("确定删除勾选的"+rows.length+"条记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/msPlanCtrl/batchDeleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"ids" : ids
