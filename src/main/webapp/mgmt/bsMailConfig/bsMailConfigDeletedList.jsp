...
<!-- 	    </div> -->
	    <div>
	        <form class="navbar-form" role="search" id="searchForm">
	            <div class="form-group">
	            	<span class="jangle-listjsp-remarks-span label label-info" >邮件配置表-已删列表 </span>
	...
	<script type="text/javascript">
		...
		var url = "/bsMailConfigCtrl/selectPage.ctrl";	//获取数据的url地址，需要实现分页功能。
		// 新增按钮 打开新增数据的页面
		function addFormInfo() {
			window.location.href = "bsMailConfigEdit.jsp"
		}
		// 查看详情
		function openDetail(data) {
			window.location.href = "bsMailConfigOpen.jsp?id=" + data.id + addressPostfix;
		}
		// 编辑详情
		function editDetail(data){
			window.location.href = "bsMailConfigEdit.jsp?id=" + data.id + addressPostfix;
		}
		// 复选框的相关功能（当启用复选框时可用）
		function doCheckBoxes(){
			...
			if (confirm("确定删除勾选的"+rows.length+"条记录吗？删除且不可恢复")) {
				$.ajax({
					type:"POST",
					url : "/bsMailConfigCtrl/batchDeleteByPrimaryKeyActually.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"ids" : ids
