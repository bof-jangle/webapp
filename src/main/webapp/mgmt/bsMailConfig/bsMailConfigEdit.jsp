...
<title>邮件配置表_编辑页面_Jangle生成工具v1.1</title>
...
<body>
	<div id="bodyTopDiv" class="jangle-editjsp-bodytopdiv" >
		<span class="label label-info">邮件配置表-内容编辑</span>
	...
	<div class="jangle-editjsp-formdiv" >
		<form id="jangleEditForm" name="jangleEditForm" class="formJ" >
			<div class="form-group" >
				<label for="mailProperty">配置属性：</label>
				<input type="text" class="form-control" id="mailProperty" name="mailProperty" placeholder="请输入配置属性" />
			</div>
			<div class="form-group" >
				<label for="mailValue">属性值：</label>
				<input type="text" class="form-control" id="mailValue" name="mailValue" placeholder="请输入属性值" />
			</div>
			<div class="form-group" >
	...
	<script type="text/javascript" src="js/bsMailConfigEdit.js"></script>
	<script type="text/javascript">
		...
		function submitForm() {
			...
			$.ajax({
				type:"POST",
				url : "/bsMailConfigCtrl/insert.ctrl",
				dataType : "json",
				cache:false,
		...
		function deleteForm() {
			if (confirm("确定删除此记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/bsMailConfigCtrl/deleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"id" : $("#id").val()
		...
		$(function() {
			...
			$.ajax({
				url : "/bsMailConfigCtrl/selectByPrimaryKey.ctrl",
				dataType : "json",
				cache : false,
				data : {
					"id" : ps["id"]
