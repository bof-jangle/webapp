...
<title>文件存储表_编辑页面_Jangle生成工具v1.1</title>
...
<body>
	<div id="bodyTopDiv" class="jangle-editjsp-bodytopdiv" >
		<span class="label label-info">文件存储表-内容编辑</span>
	...
	<div class="jangle-editjsp-formdiv" >
		<form id="jangleEditForm" name="jangleEditForm" class="formJ" >
			<div class="form-group" >
				<label for="bitContent">二进制大数据对象：</label>
				<input type="text" class="form-control" id="bitContent" name="bitContent" placeholder="请输入二进制大数据对象" />
			</div>
	...
	<script type="text/javascript" src="js/bsFileBitEdit.js"></script>
	<script type="text/javascript">
		...
		function deleteForm() {
			if (confirm("确定删除此记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/bsFileBitCtrl/deleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"id" : $("#id").val()
		...
		$(function() {
			...
			$.ajax({
				url : "/bsFileBitCtrl/selectByPrimaryKey.ctrl",
				dataType : "json",
				cache : false,
				data : {
					"id" : ps["id"]
