...
<!-- 	    </div> -->
	    <div>
	        <form class="navbar-form" role="search" id="searchForm">
	            <div class="form-group">
	            	<span class="jangle-listjsp-remarks-span label label-info" >文件存储表 </span>
				...
				<button type="button" class="btn btn-default" style="display:none;" onclick="addFormInfo()">新增</button>
				</div>
	        </form>
	    </div>
		</div>
	...
	<script type="text/javascript">
		...
		var url = "/bsFileBitCtrl/selectPage.ctrl";	//获取数据的url地址，需要实现分页功能。
		// 新增按钮 打开新增数据的页面
		function addFormInfo() {
			window.location.href = "bsFileBitEdit.jsp"
		}
		// 查看详情
		function openDetail(data) {
			window.location.href = "bsFileBitOpen.jsp?id=" + data.id + addressPostfix;
		}
		...
		function doCheckBoxes(){
			...
			if (confirm("确定删除勾选的"+rows.length+"条记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/bsFileBitCtrl/batchDeleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"ids" : ids
