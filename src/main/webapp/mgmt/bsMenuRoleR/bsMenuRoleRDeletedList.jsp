...
<title>基础框架的基础Demo_已删列表页面</title>
...
<!-- 	    </div> -->
	    <div>
	        <form class="navbar-form" role="search" id="searchForm">
	            <div class="form-group">
	            	<span class="jangle-listjsp-remarks-span label label-info" >基础框架的基础Demo-已删列表 </span>
	...
	<script type="text/javascript">
		...
		var url = "/bsMenuRoleRCtrl/selectPage.ctrl";	//获取数据的url地址，需要实现分页功能。
		// 新增按钮 打开新增数据的页面
		function addFormInfo() {
			window.location.href = "bsMenuRoleREdit.jsp"
		}
		// 查看详情
		function openDetail(data) {
			window.location.href = "bsMenuRoleROpen.jsp?id=" + data.id + addressPostfix;
		}
		// 编辑详情
		function editDetail(data){
			window.location.href = "bsMenuRoleREdit.jsp?id=" + data.id + addressPostfix;
		}
		// 复选框的相关功能（当启用复选框时可用）
		function doCheckBoxes(){
			...
			if (confirm("确定删除勾选的"+rows.length+"条记录吗？删除且不可恢复")) {
				$.ajax({
					type:"POST",
					url : "/bsMenuRoleRCtrl/batchDeleteByPrimaryKeyActually.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"ids" : ids
