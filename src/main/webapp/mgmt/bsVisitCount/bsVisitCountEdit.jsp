...
<title>访客统计_编辑页面_Jangle生成工具v1.1</title>
...
<body>
	<div id="bodyTopDiv" class="jangle-editjsp-bodytopdiv" >
		<span class="label label-info">访客统计-内容编辑</span>
	...
	<div class="jangle-editjsp-formdiv" >
		<form id="jangleEditForm" name="jangleEditForm" class="formJ" >
			...
			<div class="form-group" >
				<label for="visitCount">访问次数：</label>
				<input type="text" class="form-control" id="visitCount" name="visitCount" placeholder="请输入访问次数" />
			</div>
			<div class="form-group">
	...
	<script type="text/javascript" src="js/bsVisitCountEdit.js"></script>
	<script type="text/javascript">
		...
		function submitForm() {
			...
			$.ajax({
				type:"POST",
				url : "/bsVisitCountCtrl/insert.ctrl",
				dataType : "json",
				cache:false,
				...
				success : function(data) {
					...
					if (data.code == "10001") {
						...
						fileInputParam["attSourceType"] = "bs_visit_count"; //填业务表名称
		...
		function deleteForm() {
			if (confirm("确定删除此记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/bsVisitCountCtrl/deleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"id" : $("#id").val()
		...
		$(function() {
			...
			$.ajax({
				url : "/bsVisitCountCtrl/selectByPrimaryKey.ctrl",
				dataType : "json",
				cache : false,
				data : {
					"id" : ps["id"]
