...
<title>邮件发送记录表_编辑页面_Jangle生成工具v1.1</title>
...
<body>
	<div id="bodyTopDiv" class="jangle-editjsp-bodytopdiv" >
		<span class="label label-info">邮件发送记录表-内容编辑</span>
	...
	<div class="jangle-editjsp-formdiv" >
		<form id="jangleEditForm" name="jangleEditForm" class="formJ" >
			<div class="form-group" >
				...
				<input type="text" class="form-control" id="toEmail" name="toEmail" placeholder="请输入邮箱收件人" />
			</div>
			<div class="form-group" >
				<label for="mailSubject">邮件标题：</label>
				<input type="text" class="form-control" id="mailSubject" name="mailSubject" placeholder="请输入邮件标题" />
			</div>
			<div class="form-group" >
	...
	<script type="text/javascript" src="js/bsMailEdit.js"></script>
	<script type="text/javascript">
		...
		function deleteForm() {
			if (confirm("确定删除此记录吗？")) {
				$.ajax({
					type:"POST",
					url : "/bsMailCtrl/deleteByPrimaryKey.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"id" : $("#id").val()
